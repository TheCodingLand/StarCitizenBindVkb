...
class GameAction(CustomBaseModel):
    ...
    @classmethod
    def check_xml_errors(cls, data: Dict[str, Any]) -> Dict[str, Any]:
...
def get_sc_actionmaps_path(
    install_type: Literal["LIVE", "PTU", "EPTU"] = "LIVE",
    sc_version: str = "sc-alpha-3.24.2-9381373",
) -> Path:
    return APP_PATH / "data" / install_type / sc_version / "actionmap.json"


def get_all_defined_game_actions(
    sc_actionmaps_path: Path = get_sc_actionmaps_path(),
) -> Dict[str, List[GameAction]]:
    # returns action names str, adds main_category and sub_category to the actionmap
    output: Dict[str, List[GameAction]] = {}
...
def get_all_subcategories_actions(
    sc_actionmaps_path: Path = get_sc_actionmaps_path(),
) -> AllActionMaps:
    actionmaps: Dict[str, Dict[str, Dict[str, str]]] = json.loads(sc_actionmaps_path.read_text())
    aam = AllActionMaps.model_validate(actionmaps)
...
if __name__ == "__main__":
    actions_by_name = get_all_defined_game_actions()
    print(actions_by_name)
    for actions in actions_by_name.values():
        for game_action in actions:
            joystick = game_action.joystick
            if isinstance(joystick, DeviceActivation):
                if joystick.input not in {" ", "", None}:
                    print(game_action.name)
                    print(joystick)
