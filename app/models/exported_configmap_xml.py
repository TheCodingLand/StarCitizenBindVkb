...
def get_action_maps_file(source_file: Union[str, Path]) -> Dict[str, Any]:
...
class JoystickBind(BaseModel):
    ...
    modifier: Optional[Literal["HOLD", "DOUBLE_TAP", "MODIFIER"]]
    action: str
    ...
    def from_rebind(cls, bind: Rebind) -> "JoystickBind":
        ...
        if "+" in bind.input:
            ...
            modifier = None
        return cls(
            axis_or_button=bind.input, modifier_str=modifier_str, modifier=modifier, action=action
        )
...
if __name__ == "__main__":
    ...
    action_maps_dict = get_action_maps_file(action_maps_file_souce)
    name_action_mapping = {
        action.name: action for actions in all_defined_game_actions.values() for action in actions
    }
    action_maps = ActionMapsFile(**action_maps_dict)
    if action_maps.action_profiles:
        for option in action_maps.action_profiles[0].actionmap:
